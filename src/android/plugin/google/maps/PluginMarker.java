...
public class PluginMarker extends MyPlugin implements MyPluginInterface  {
  ...
  /**
   * Prepapre MarkerOptions for create()
   *
   * @param  opts
   * @return
   ...
   * @param opts
   * @param useCache
   *@param callback  @throws JSONException
   */
  private void createInstance(final JSONObject opts, final boolean useCache, final PluginAsyncInterface callback) throws JSONException {
  ...
  public void create(final JSONObject markerOptions, final CallbackContext callbackContext) throws JSONException {
    this.createInstance(markerOptions, false, new PluginAsyncInterface() {
      @Override
      public void onPostExecute(Object object) {
  ...
  public void createCollection(final JSONArray markerOptionsCollection, final CallbackContext callbackContext) throws JSONException {
    ...
    for (int i = 0; i < markerOptionsCollection.length(); i++) {
      ...
      this.createInstance(markerOptions, true, new PluginAsyncInterface() {
        @Override
        public void onPostExecute(Object object) {
          JSONObject result = (JSONObject) object;
  ...
  private void setIconFromBitmap(Marker marker, final Bundle iconProperty, Bitmap image, final PluginAsyncInterface callback) {
    if (image == null) {
      callback.onPostExecute(marker);
      return;
    }
...
//      image.recycle();
  ...
  private Bitmap loadLocalIcon(Bundle iconProperty, boolean useCache) {
    ...
    if (Bundle.class.isInstance(size)) {
      ...
      width = sizeInfo.getInt("width", 0);
      height = sizeInfo.getInt("height", 0);
      ...
      if (width > 0 && height > 0) {
        ...
        // add dimension to icon key name
        cacheIconKey += "-" + width + "_" + height;
      }
    }
...
//    icons.add(image);
    ...
    if (isResized) {
      Bitmap resizedImage = PluginUtil.resizeBitmap(image, width, height);
      image.recycle();

      image = resizedImage;
    } else {
      Bitmap scaledImage = PluginUtil.scaleBitmapForDevice(image);
      image.recycle();
...
//    icons.add(image);
    ...
    if (useCache) {
      ...
      this.localImageCache.addBitmapToMemoryCache(cacheIconKey, image);
    }
  ...
  private AsyncTask prepareIconLoadingFromLocalTask(final Marker marker, final Bundle iconProperty, final PluginAsyncInterface callback, final boolean useLocalCache) {
    return new AsyncTask<Void, Void, Bitmap>() {
      ...
      protected Bitmap doInBackground(Void... params) {
        return PluginMarker.this.loadLocalIcon(iconProperty, useLocalCache);
      }
  ...
  private String prepareIconUrl(String iconUrl) {
    if (iconUrl == null) {
      return iconUrl;
    }
